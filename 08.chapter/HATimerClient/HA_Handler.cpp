...
HA_Handler::~HA_Handler(void)
{
  if(timer_id_ != 0)
  {
    proactor()->cancel_timer(timer_id_); 
    timer_id_ = 0; 
  }
...
void HA_Handler::open(ACE_HANDLE new_handle, ACE_Message_Block& message_block)
{
  ...
  timer_id_ = proactor()->schedule_timer(*this, 0, ACE_Time_Value(1), ACE_Time_Value(1)); 
  if(timer_id_ < 0)
  {
    ACE_ERROR((LM_ERROR, ACE_TEXT("%p\n"), ACE_TEXT("schedule timer"))); 
    delete this; 
    return; 
  }
}
...
void HA_Handler::handle_time_out(const ACE_Time_Value& tv, const void* act)
{
