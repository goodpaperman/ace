...
int runUnboundedStackWithAllocator(ACE_Allocator *allocator)
{
  ACE_TRACE(ACE_TEXT("runUnboundedStackWithAllocator")); 
  ACE_DEBUG((LM_DEBUG, ACE_TEXT("Using an unbounded stack\n"))); 
  //ACE_Unbounded_Stack<DataElement*> ustack; 
  ...
  for(int m=0; m<100; ++m)
  {
    ...
    //ACE_NEW_RETURN(elem, DataElement(m), -1); 
    //ustack.push(elem); 
    if(ustack.push(DataElement(m)) == -1)
      ACE_ERROR_RETURN((LM_ERROR, ACE_TEXT("%p\n"), ACE_TEXT("push next element")), -1); 
  }

  void* furtherMemory = allocator->malloc(sizeof(ACE_Node<DataElement>)); 
  ...
  furtherMemory = allocator->malloc(sizeof(ACE_Node<DataElement>)); 
  ACE_ASSERT(furtherMemory != 0); 
  if(furtherMemory != 0)
    ACE_DEBUG((LM_DEBUG, ACE_TEXT("%p\n"), ACE_TEXT("Has memory..."))); 
...
int ACE_TMAIN(int argc, ACE_TCHAR* argv[])
{
  ...
  ACE_Dynamic_Cached_Allocator<ACE_Null_Mutex> *allocator; 
  ACE_NEW_RETURN(allocator, ACE_Dynamic_Cached_Allocator<ACE_Null_Mutex>(101, block_size), -1); 

  //testContainerSize(); 
  //runBoundedStack(); 
  //runFixedStack();
  //runUnboundedStack(); 
  runUnboundedStackWithAllocator(allocator); 
  //runUnboundedQueue(); 
