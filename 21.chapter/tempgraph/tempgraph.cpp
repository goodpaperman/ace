...
public:
    ...
    void reset ()
    {
        ...
        ACE_OS::srand (ACE_OS::thr_self () + factor ++); 
    }
}; 
...
public:
    ...
    int parse_args (int argc, ACE_TCHAR *argv[])
    {
        ...
        u_short port = 10003; 
        ACE_Get_Opt opt (argc, argv, "a:p:i:r:m:s:"); 
        while ((c = opt ()) != -1)
        {
            switch (c)
            {
        ...
        addr_.set (host, port); 
        ACE_DEBUG ((LM_DEBUG, "interval = %d, reset = %d, max = %d, size = %d\n", interval_, reset_, max_, size_)); 
        return 0; 
    }
    ...
    int history_value () { return size_; } 
...
public:
    Temperature_Monitor (
        ...
        Naming_Context &ctx, 
        Naming_Context &shctx)
        : opt_ (opt), ctx_ (ctx), shctx_ (shctx) 
    {
    ...
    void record_history (float temp)
    {
        ...
        this->shctx_.rebind ("lastUpdate", now); 

        //Name_Binding_Ptr cnt = this->shctx_.fetch ("counter"); 
        Name_Binding_Ptr cnt (this->shctx_.fetch ("counter")); 
        int count = cnt.get () ? cnt->int_value () : 0; 

        char name[BUFSIZ] = { 0 }; 
        ACE_OS::sprintf (name, "history[%d]", count); 

        char value[BUFSIZ] = { 0 }; 
        ACE_OS::sprintf (value, "%d|%.2f", now, temp); 

        this->shctx_.rebind (name, value); 
        count = ++ count % this->opt_.history_value (); 
        this->shctx_.rebind ("counter", count); 
    }
...
int ACE_TMAIN (int argc, ACE_TCHAR* argv[])
{
    ...
    {
        ACE_Name_Options *options = shctx.name_options (); 
#if defined (USE_NET)
...
#endif 
        shctx.open (options->context ()); 
    }

    Temperature_Monitor mnt (opt, ctx, shctx); 
    mnt.monitor (); 
